...
- [ ] T006 [P] Create `src/data/services.ts` (service records: capabilityId, treatment, launchSet, searchIntent, deliverables, connections, specialist tools, faqIds, related, action label) with canonical-placement rules.
  - Refs FR-015/016, contracts/content-schemas.md · Deps none · Parallel T005/T007–T010 · Validate integrity test · Done when every service maps to exactly one capability and placement rules hold.
...
- [ ] T019 [US1] Implement the full homepage section sequence in `src/pages/index.astro` from `src/data/home.ts` (Hero → Immediate value → Problem → Three starting points → Anatomy → Growth Graph → Six capabilities → Start with the problem → Modern search and AI → How we work → Work and proof → Why → FAQ → Final CTA) using approved playbook copy.
  - Refs FR-007/035/038 · Deps Gate A · Parallel none · Validate render test + copy-lint · Done when all sections render in order with brand promises intact.
...
- [ ] T022 [US3] Implement the Growth Graph homepage centrepiece (dark environment) in `src/components/brand/GrowthGraph.astro` + `src/data/growthGraph.ts` with five goals including "Save team time".
  - Refs FR-009/020/021 · Deps Gate A · Parallel none · Validate `tests/unit/growthgraph.test.ts` · Done when five goals render with real relationships only.
...
- [ ] T034 [US4] Populate `services.ts` with the seven launch services (Ecommerce Development, Website Design & Development, SEO & Search Visibility, Paid Advertising, Conversion Optimisation, Email Marketing, Analytics & Tracking) and enforce canonical placement (Landing Pages under Convert; CRM under Connect; AI-search a section of SEO; testing canonical on CRO; Amazon under Paid Advertising).
  - Refs FR-015/016, data-model.md · Deps T033 · Parallel none · Validate placement test · Done when all seven map correctly and rules hold.
...
- [ ] T042 [US3] Build the How It Connects hub in `src/pages/how-it-connects.astro` + `src/data/howItConnects.ts` (five links, worked Growth Graph examples, anatomy of a disconnected sale, self-check), understandable without motion.
  - Refs FR-019, SC-012 · Deps Gate A, T022 · Parallel none · Validate no-JS + render e2e · Done when the page is fully readable without animation.
- [ ] T043 [US3] Present the Growth Graph Review offer as a genuine diagnostic (map, gaps, next steps, starting scope), not an instant revenue estimate or disguised sales call.
  - Refs FR-022 · Deps T042 · Parallel none · Validate copy-lint (claim safety) · Done when framing is diagnostic and claim-safe.
- [ ] T044 [US2] Build the Map Your Stack page `src/pages/map-your-stack.astro` reusing `StackGraph.astro`, `stackEval.ts`, `stackRules.ts`, `stackTools.ts` (semantic fieldset/legend, real checkboxes).
  - Refs FR-023, contracts/map-your-stack.md · Deps Gate A · Parallel none · Validate a11y + render e2e · Done when nine tools select accessibly and drive the graph.
- [ ] T045 [US2] Implement/confirm pure rule evaluation and scoring in `src/lib/stackEval.ts` (in place/gap/not relevant, OR-groups, zero-relevant safe, bands at 0/40/70/100, top-3 gaps, top-2 working, priority order).
  - Refs FR-023, contracts/map-your-stack.md · Deps T044 · Parallel none · Validate `tests/unit/stackeval.test.ts` · Done when all boundary cases pass.
- [ ] T046 [US2] Implement live graph + reveal: solid vs gap links distinct beyond colour, reveal gated at ≥2 tools, result announced via live region, reduced-motion final state, graph text equivalent; no invented figures.
...
- [ ] T047 [US2] Implement the allowlist-parsed optional handoff to `/book-a-call` (prefills the context field; page works with no parameter; tamper-safe).
  - Refs FR-025, SC-009, Edge Cases · Deps T045/T048 · Parallel none · Validate handoff + tamper e2e · Done when a valid selection prefills and unknown values are ignored.
- [ ] T048 [US1] Implement the consultation form in `src/components/forms/BookingForm.astro` + `src/lib/booking.ts`: required name/work-email/business/privacy, no budget field, blur+submit validation with accessible errors, honeypot, explicit submitting state (no duplicate submit), provider-neutral never-false-success submission, email fallback.
  - Refs FR-026/027, contracts/consultation-form.md · Deps Gate A · Parallel none · Validate `tests/unit/booking.test.ts` · Done when submitting/accepted/failure states behave and no false success occurs.
- [ ] T049 [US1] Implement the honest confirmation (three outcomes, no time promise) and recoverable-failure state; document the data flow (no server-side storage; email delivery via the form processor) and Privacy disclosure hook.
  - Refs FR-028/047, SC-010 · Deps T048 · Parallel none · Validate confirmation e2e · Done when confirmation carries the three-outcomes message and answers persist on failure.
- [ ] T050 [US1] Add the `/contact` route `src/pages/contact.astro` (name, work email, message; shared submission path; no qualification fields or handoff).
  - Refs FR-029, contracts/consultation-form.md · Deps T048 · Parallel none · Validate contact submit e2e · Done when `/contact` posts on the shared never-false-success path.
- [ ] T051 [P] Implement the newsletter opt-in with submitting/success/error states behind `PUBLIC_NEWSLETTER_ENDPOINT`; the footer omits it until configured (confirmed double opt-in).
  - Refs FR-050 · Deps T013/T048 · Parallel T052 · Validate newsletter state test · Done when the opt-in is absent unconfigured and honest when enabled.
- [ ] T052 [P] Add/extend tests: `stackeval`, `booking` (submitting, never-false-success, contact, newsletter), handoff allowlist, accessible form errors.
  - Refs plan Testing strategy · Deps T045–T051 · Parallel T051 · Validate `npm run test` · Done when tests pass.

**Checkpoint (Gate F)**: Both interactives are accessible and honest; consultation and contact never fake success; newsletter is honestly deferred; handoff is tamper-safe.
...
- [ ] T053 [US6] Build the Work index `src/pages/work/index.astro` with capability and audience filters, an honest empty state, and an honest interim state while proof is limited.
...
- [ ] T054 [US6] Build the case-study template `src/pages/work/[slug].astro` from `content/work` (six-part narrative, scoped graph, related-service links, permission state).
  - Refs FR-031 · Deps T053 · Parallel none · Validate schema + render test · Done when a case study renders the six-part structure with a scoped graph.
- [ ] T055 [P] [US6] Add anonymized proof with placeholder connection-map art carrying honest alt text (never implies a real client/result); no invented figures anywhere.
  - Refs FR-037/048, SC-007 · Deps T054 · Parallel T061 · Validate honesty lint + alt-text check · Done when anonymized art is honest and figure-free.
- [ ] T056 [US7] Build the Insights index `src/pages/insights/index.astro` with topic categories and canonicalised category/tag views (not indexable thin pages).
...
- [ ] T057 [US7] Build the article template `src/pages/insights/[slug].astro` (question title, honest H1, key points, plain answer, pitfalls without fear language, self-check, sources, last-reviewed date, cluster links to 1 capability + 1 audience + 2 articles).
  - Refs FR-032, contracts/content-schemas.md · Deps T056 · Parallel none · Validate schema + link-rule test · Done when an article satisfies the cluster link rule and carries sources + review date.
- [ ] T058 [US8] Build About `src/pages/about.astro` (two-audience story, operating philosophy, plain five-step process, honest credentials; real people/photos where available, no invented awards/counts/superlatives).
...
- [ ] T059 [P] Build the FAQ hub `src/pages/faq.astro` with single-open accordions (expanded state maintained, accessible).
  - Refs FR-034 · Deps T012 · Parallel T060 · Validate accordion a11y e2e · Done when the hub is keyboard-operable with announced state.
- [ ] T060 [P] Build utility/legal shells `src/pages/privacy.astro`, `cookies.astro`, `terms.astro` (interface copy) with the Privacy data-handling disclosure (no server-side storage; email delivery; retention).
  - Refs FR-047, Assumptions (legal deferred) · Deps Gate A · Parallel T059 · Validate render test · Done when shells ship interface copy and Privacy discloses the enquiry data flow.
- [ ] T061 [P] Add tests: content-collection schema shape, Work/Insights empty and canonical states, no-invented-figures lint.
  - Refs plan Testing strategy · Deps T054/T057 · Parallel T055 · Validate `npm run test` · Done when tests pass.
...
- [ ] T062 Run the per-page metadata sweep across all page types (single H1 with the main promise, logical heading order, unique title/description, one canonical; filtered views canonicalise).
  - Refs FR-039/040, SC-011 · Deps Phases 2–7 · Parallel T063/T064 · Validate `tests/unit/seo.test.ts` + audit · Done when every indexable page satisfies the metadata contract.
- [ ] T063 [P] Apply and validate visible-only structured data across page types (Organization, WebSite, WebPage, BreadcrumbList, Article, Service/Product where visible).
  - Refs FR-040, contracts/seo-metadata.md · Deps Phases 2–7 · Parallel T062/T064 · Validate schema test · Done when schema matches visible content only.
- [ ] T064 [P] Verify the sitemap (public routes, excludes 404) and robots allowlist end to end.
  - Refs FR-041 · Deps Phase 1 · Parallel T062/T063 · Validate robots/sitemap test · Done when allow/block and exclusion verify on the build.
- [ ] T065 [US9] Run the accessibility gate: axe WCAG 2.2 AA on audited page types, keyboard operability (chrome, both islands, forms, accordions, tabs), outline focus, live regions, contrast matrix, and the alt-text policy.
...
- [ ] T066 [US9] Run the responsive gate at 320/360/390/768/1024/desktop: no horizontal scroll, 44px targets, radial ≥720 / vertical <720, primary CTA never buried.
...
- [ ] T067 [US9] Verify no-JS and reduced-motion parity across pages and both islands (all meaning present; final states shown).
...
- [ ] T068 [P] Run the security gate: CSP verified against the consultation and contact submit paths, headers present, zero third-party JS at load, allowlist handoff, no hardcoded secrets or committed enquiry data.
  - Refs FR-047/049, contracts/security-headers.md · Deps Phase 1, Phase 6 · Parallel T069/T070/T071 · Validate header/CSP check · Done when submissions succeed under CSP and no secret is exposed.
- [ ] T069 [P] Run the performance gate: build asset sizes vs budgets (JS 30/50 KB, CSS 40 KB, fonts 200 KB, LCP image 150 KB, initial page 500 KB), Lighthouse mobile 90+, CWV; document any overage.
  - Refs FR-051, contracts/performance-budgets.md, SC-006 · Deps Phases 2–7 · Parallel T068/T070/T071 · Validate Lighthouse + size check · Done when budgets are met or overage is documented with exact scores.
- [ ] T070 [P] Run the copy-honesty lint across data modules and rendered output (no em dashes, superlatives, buzzwords, flagged claims, invented figures).
  - Refs FR-036/037, SC-007 · Deps Phases 2–7 · Parallel T068/T069/T071 · Validate content-lint test · Done when the ban list is clean sitewide.
- [ ] T071 [P] Capture visual-QA screenshots at 1440/1024/768/390/360 (reducedMotion) for key page types and both islands' states; confirm native feel, distinct gap vs active links, no overflow/overlap/clipping, no false-audit claims.
  - Refs quickstart.md Screenshot QA · Deps Phases 2–7 · Parallel T068/T069/T070 · Validate manual screenshot review · Done when all captures are clean.
...
- [ ] T072 Run the full `quickstart.md` validation (all nine scenarios) against the production build/preview.
...
- [ ] T073 Validate cross-artifact content integrity: all internal links resolve, CTA labels match destinations, breadcrumbs correct, no dead ends in any core journey.
...
- [ ] T074 Audit owner-supplied dependencies and honest fallbacks (fonts, proof, people/photos, legal, `PUBLIC_*` endpoints); confirm nothing structural is blocked.
...
- [ ] T075 Verify the Definition of Done against the constitution's fourteen principles (mobile, speed/budgets, design coherence, humanized copy, SEO, a11y, security, tests, build/lint, no stray placeholders, documented performance).
  - Refs §XIV, plan Constitution Check · Deps T072–T074 · Parallel none · Validate DoD checklist · Done when all gates verify.
- [ ] T076 Run `/speckit-converge` to assess the build against spec/plan and append any residual work as new tasks.
  - Refs plan Phase I, CLAUDE.md workflow · Deps T075 · Parallel none · Validate converge report · Done when residual work is recorded.
...
- **Phase 5**: T039 ∥ T041.
- **Phase 6**: T051 ∥ T052.
- **Phase 7**: T055, T059, T060, T061 ∥.
- **Phase 8**: T062/T063/T064 ∥; T068/T069/T070/T071 ∥.
...
Gate A foundation (**T001→T004→T011→T013→T017→T018**) → capability system (**T027→T028**) → launch services (**T033→T034**) → consultation + interactives (**T048→T049**, **T044→T045→T046→T047**) → cross-cutting gates (**T065, T066, T069**) → convergence (**T072→T075→T076**). Phases 2, 5 and 7 branch off Gate A / Gate C and rejoin at Phase 8, so they are not on the critical path if staffed in parallel.
...
2. Phase 2 (Homepage + Growth Graph) and the Phase 6 consultation tasks (T048–T050) → an understandable homepage that converts.

...
### Session 2026-07-11 (spec refinements)

Seven minor gaps raised by the project checklist were resolved before task generation:

- Q: `/contact` vs `/book-a-call` → A: `/book-a-call` is the primary conversion (a structured free-consultation request with light optional qualification and the Map Your Stack handoff); `/contact` is a lighter general-enquiry route (name, work email, message) for non-consultation contact. Both share the never-false-success submission path; "Book a free call" stays the sitewide primary CTA (FR-029).
- Q: Consultation-enquiry data flow, storage and retention → A: The static site stores nothing server-side; a submission is sent over HTTPS to the provider-neutral endpoint, which delivers it to the owner by email; any at-rest copy lives only with the configured form processor and the owner's inbox, retained only as long as needed to respond; the Privacy page discloses this (FR-047).
- Q: Alt-text and non-text-content accessibility policy → A: Informational images and screenshots carry descriptive alt text; decorative images and icons are marked decorative; the two interactives expose meaning as text with decorative SVG parts hidden; anonymized work artwork uses honest alt text that never implies a real client or result (FR-048).
- Q: Security headers and CSP → A: Baseline headers via the host headers file, including a Content Security Policy scoped to self plus the configured form-endpoint origin, plus nosniff, a strict Referrer-Policy, a restrictive Permissions-Policy and HSTS where the host provides it (FR-049).
- Q: Newsletter success, submitting and error states → A: The newsletter follows the same never-false-success contract with accessible submitting, success and error states and confirmed (double) opt-in; until a provider endpoint is configured the newsletter is deferred and the footer omits the opt-in rather than show a form that cannot deliver (FR-050).
- Q: JavaScript and asset performance budgets → A: Measurable per-page mobile budgets (JS 30 KB content / 50 KB interactive pages, CSS 40 KB, fonts 200 KB total, LCP image 150 KB, initial page 500 KB, zero third-party JS at load, healthy Core Web Vitals) verified at the hardening gate (FR-051).
- Q: Consultation-form submitting state → A: An explicit, accessible submitting state (disabled, busy-announced control showing progress as text) that prevents duplicate submissions and resolves to the confirmation or recoverable-failure state (FR-027).
...
- **Submission in flight**: the form shows an accessible submitting state and ignores repeated activation so a slow network cannot produce a duplicate enquiry; the state resolves to the confirmation or the recoverable-failure state.
- **Newsletter provider not configured**: the footer omits the opt-in entirely rather than show a form that cannot deliver; when a provider is configured the opt-in appears with submitting, success and error states and confirmed opt-in.
...
- **FR-004**: The footer MUST present the brand block (infinity mark, wordmark, slogan and company description), a primary CTA, link columns (What we do, Who we help, Learn, Company, including a `/contact` link), a newsletter opt-in when a newsletter provider is configured (deferred otherwise, per FR-050), and a legal row (Privacy, Cookies, Terms).
...
- **FR-027**: The form MUST validate on blur and on submit with specific, accessible errors (icon plus text, never colour alone), include spam protection, and submit through a provider-neutral path that never reports a false success and offers a direct email fallback on failure. During submission the form MUST enter an explicit, accessible submitting state (a disabled, busy-announced control that shows progress as text, not colour or motion alone) that prevents duplicate submissions and resolves to either the confirmation or the recoverable-failure state.
...
- **FR-029**: Two written enquiry surfaces MUST be distinguished. `/book-a-call` is the primary conversion: a structured free-consultation request with light optional qualification and the Map Your Stack handoff. `/contact` is a lighter general-enquiry route (a short name, work email and message) for questions, partnership, press or existing-client contact that are not a consultation request; it carries no qualification fields and no Map Your Stack handoff. Both share the provider-neutral, never-false-success submission path and the submitting/confirmation/failure states; the primary CTA sitewide remains "Book a free call" to `/book-a-call`, and `/contact` is reached from the footer and utility links rather than promoted as the primary conversion. No calendar-scheduling widget is included at launch: the written enquiry is the only submission path and any call is arranged in the personal follow-up; an optional scheduler MAY be added later behind the same primary conversion. The newsletter MUST be opt-in and separate from both enquiry surfaces (see FR-050).
...
**Data handling, security headers, alt text, newsletter and performance budgets (resolved 2026-07-11)**

- **FR-047**: The consultation and contact enquiry data flow MUST be documented and honest: the static site stores no enquiry data server-side; a submission is validated client-side and sent over HTTPS to the provider-neutral endpoint, which delivers it to the owner by email; any at-rest copy lives only with the configured form processor and the owner's inbox. The site MUST request only the data needed to reply (no special-category data), and the Privacy page MUST disclose what is collected, that a third-party form processor delivers it, and that it is retained only as long as needed to respond and per the processor's policy.
- **FR-048**: Every non-text element MUST carry an appropriate text alternative: informational images and screenshots MUST have descriptive alt text; purely decorative images and icons MUST be marked decorative (empty alt or hidden from assistive technology); the Growth Graph and Map Your Stack MUST expose their meaning as text (accessible name and announced state) with decorative SVG sub-parts hidden; and anonymized work artwork MUST use honest alt text that never implies a real client, screenshot or result.
- **FR-049**: The site MUST send baseline security response headers (via the host headers file): a Content Security Policy that restricts default, script, style, image, font, connect and form-action sources to self plus the configured form-endpoint origin (and data: images), with frame-ancestors none, base-uri self and object-src none; plus X-Content-Type-Options nosniff, a strict Referrer-Policy, a restrictive Permissions-Policy, and HSTS where the host provides it. The CSP MUST allow the consultation and contact submissions to reach the configured endpoint and MUST NOT require inline third-party scripts.
- **FR-050**: The newsletter opt-in MUST follow the same provider-neutral, never-false-success contract as the enquiry. It MUST define an accessible submitting state, a success state and an error state, and MUST use confirmed (double) opt-in. Until a newsletter provider endpoint is configured, the newsletter is deferred: the footer MUST omit the opt-in rather than present a form that cannot deliver.
- **FR-051**: The build MUST meet measurable per-page mobile performance budgets (compressed transfer, verified at the hardening gate): JavaScript at most 30 KB on content pages and 50 KB on the two interactive pages; CSS at most 40 KB; self-hosted subset fonts at most 200 KB total; the LCP image at most 150 KB and initial page weight at most 500 KB excluding below-fold lazy media; zero third-party JavaScript at initial load; and Core Web Vitals in the healthy range (LCP under 2.5s, INP under 200ms, CLS under 0.1). Budgets are best-effort ceilings consistent with the Lighthouse 90+ target and any overage MUST be documented.
...
- **Consultation enquiry**: the submitted request. Attributes: name, work email, business name, optional website, goal, obstacle, closest-description, working model, optional timing, optional file, carried Map Your Stack context, required privacy acknowledgement; plus submission state (idle, submitting, accepted, recoverable failure) and the follow-up expectation. No budget field is collected. The site stores it nowhere server-side; it is delivered to the owner by email through the configured form processor and retained only per that processor and the owner's inbox.
- **Contact enquiry**: a lighter general message (name, work email, message) for non-consultation contact (questions, partnership, press, existing-client). Shares the never-false-success submission path and the submitting/success/failure states; carries no qualification fields or Map Your Stack handoff.
...
- **Owner-supplied assets are dependencies, not blockers for structure**: licensed Geist WOFF2 fonts, the production domain (`PUBLIC_SITE_URL`), booking endpoint (`PUBLIC_BOOKING_ENDPOINT`) and, when the newsletter is enabled, a newsletter endpoint (`PUBLIC_NEWSLETTER_ENDPOINT`), founder and team information and photography, at least three real work examples and two or three permissioned case studies, testimonials, verifiable credentials, and legal wording for privacy, cookies and terms. The site behaves honestly (safe fallbacks, honest interim states, no invented proof) until they arrive.
- **Enquiry data handling** (confirmed 2026-07-11): the site stores no enquiry data server-side; submissions are delivered to the owner by email through the configured form processor and retained only per that processor and the owner's inbox; the Privacy page discloses this and only the data needed to reply is requested. The `/contact` general-enquiry route is distinct from the `/book-a-call` consultation conversion.
- **Security headers and CSP** (confirmed 2026-07-11): baseline security response headers ship via the host headers file (`public/_headers` on Cloudflare Pages), including a Content Security Policy scoped to self plus the configured form-endpoint origin. The exact endpoint origins are wired from the same env config as the form.
- **Newsletter** (confirmed 2026-07-11): deferred until a provider endpoint is configured; the footer omits the opt-in until then, and when enabled it uses accessible submitting/success/error states with confirmed opt-in and the never-false-success contract.
- **Performance budgets** (confirmed 2026-07-11): measurable per-page mobile budgets (FR-051) are the ceilings verified at the hardening gate alongside the Lighthouse target; overage is documented rather than hidden.
